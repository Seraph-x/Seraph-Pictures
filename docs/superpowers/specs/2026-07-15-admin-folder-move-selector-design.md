...
Use an Element UI dialog containing a filterable, creatable `el-select`. This fits the existing Vue 2 and Element UI Admin interface, provides native keyboard filtering, and avoids building a separate autocomplete or tree component.
...
The administrator can search the options by typing, choose an existing path, choose the root directory, or finish typing a new path. Confirming normalizes the selected value with the existing `normalizeFolderPath` function. Cancelling closes the dialog without changing selection or issuing a request.

While the move request is active, the confirm action is disabled or loading to prevent duplicate submissions. On success, the existing localized success message reports the number of moved files and the normalized destination. Selecting the current folder remains a no-op and uses the existing informational message.
...
Expose a computed destination list derived immutably from `folders`. Normalize and deduplicate paths, sort them with the same path ordering already used by the folder sidebar, and prepend the root option. Do not mutate the `folders` collection while building options.
...
- same-folder selection remains a no-op;
